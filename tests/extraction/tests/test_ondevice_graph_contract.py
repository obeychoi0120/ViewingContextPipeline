...
from viewing_context_pipeline.extraction.scene_context_extraction.ondevice.pipeline import (
    SceneContextJob,
...
from viewing_context_pipeline.extraction.scene_context_extraction.graph_adapter.payload import select_scene_image_paths
from viewing_context_pipeline.extraction.scene_context_extraction.graph_adapter.writer import build_graph_record
from viewing_context_pipeline.extraction.scene_context_extraction.graph_core.fingerprint import (
...
class OnDeviceGraphContractTests(unittest.TestCase):
    ...
    def test_single_and_parallel_execution_write_the_same_schema(self) -> None:
        class InProcessWorkerPool:
            ...
            def run_tasks(self, tasks, on_task_complete=None):
                from viewing_context_pipeline.extraction.scene_context_extraction.ondevice.extractor import extract_visual_graphs
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch(
                "viewing_context_pipeline.extraction.scene_context_extraction.ondevice.extractor.extract_scene_graph",
    ...
    def test_complete_scene_context_rebuilds_missing_profile_without_inference(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            frames.mkdir()
            Image.new("RGB", (672, 384)).save(frames / "0000.png")
            multimodal_ref = root / "ref.jsonl"
            multimodal_ref.write_text(json.dumps({"scene_idx": 0}) + "\n", encoding="utf-8")
            ...
            job = SceneContextJob(
                content_id="demo",
                multimodal_ref=str(multimodal_ref),
            ...
            write_fingerprint(
                ...
                build_input_fingerprint(
                    content_id="demo",
                    scenes=[{"scene_idx": 0, "keyframe_timestamps": [0]}],
                    frames_dir=frames,
                    multimodal=False,
                    backend="ondevice",
        ...
        self.assertEqual(context["content_id"], "demo")
