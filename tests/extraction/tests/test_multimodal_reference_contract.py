...
from viewing_context_pipeline.extraction.scene_context_extraction.graph_adapter.payload import get_keyframe_timestamps
from viewing_context_pipeline.extraction.data_preparation.data_processor import merge_scene_data


class MultimodalReferenceContractTests(unittest.TestCase):
    def test_timeline_uses_filtered_keyframe_timestamps(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            timestamps = root / "timestamp_filtered.json"
            output = root / "demo_multimodal_ref.jsonl"
    ...
    def test_generation_rejects_mismatched_shots_and_keyframes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            timestamps = root / "timestamp_filtered.json"
            timestamps.write_text(
            ...
            with self.assertRaisesRegex(ValueError, "shot intervals"):
                merge_scene_data(timestamps, "", "", root / "demo_multimodal_ref.jsonl")
    ...
    def test_ondevice_uses_multimodal_timeline_timestamps_first(self) -> None:
