...
@pytest.fixture
def full_context(ready_context, fake_models, generate_all):
    generate_all(ready_context)
    prepare_validation_cohort(ready_context)
...
def prepare_embeddings(context):
    ...
    write_json(
        context.representations_dir / "item_index.json", {str(i): i - 1 for i in range(1, 5)}
    )
...
def test_spawned_training_matches_serial_parameters_and_metrics(full_context):
    ...
    prepare_validation_cohort(serial)
    ...
    try:
        for split, identity, branch in jobs:
            run_combination(
                ...
                identity,
                branch,
                prepare_split(table, split),
                torch.device("cpu"),
            )
            parallel_dir = combination_dir(
                context, split["evaluation_date"], identity["seed"], identity["arm"]
            )
            serial_dir = combination_dir(
                serial, split["evaluation_date"], identity["seed"], identity["arm"]
            )
            for filename in ("per_event_metrics.jsonl",):
                assert (parallel_dir / filename).read_bytes() == (
                    serial_dir / filename
                ).read_bytes()
            complete_a, complete_b = (
                read_json(path / "complete.json") for path in (parallel_dir, serial_dir)
            )
...
def test_parent_interrupt_keeps_completed_artifacts_for_resume(full_context, monkeypatch):
    ...
    assert {child.pid for child in mp.active_children()} == children_before
    protected = {
        path: path.read_bytes() for path in context.recommendations_dir.rglob("complete.json")
    }
    assert protected
    monkeypatch.setattr(
        "validation.rolling_recommendation.worker_devices", lambda *_: ["cpu", "cpu"]
    )
    pending = []
    ...
    monkeypatch.setattr(
        "validation.rolling_recommendation.verify_representations", lambda *a, **k: None
    )
    monkeypatch.setattr(
    ...
    assert all(
        combination_dir(context, split["evaluation_date"], identity["seed"], identity["arm"])
        / "complete.json"
        not in protected
        for split, identity, _ in pending
    )
...
def test_dispatch_is_unique_and_skips_completed_work(full_context, monkeypatch):
    ...
    prepare_embeddings(context)
    monkeypatch.setattr(
        "validation.rolling_recommendation.worker_devices", lambda *_: ["cuda:0", "cuda:1"] * 2
    )
    reused = ("2022-09-05", 42, "meta")
    monkeypatch.setattr(
        "validation.rolling_recommendation.combination_complete",
    ...
    def run(context, jobs, devices, progress):
        ...
        keys = [
            tuple(identity[k] for k in ("evaluation_date", "seed", "arm"))
