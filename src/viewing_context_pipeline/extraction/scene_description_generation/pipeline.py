...
from viewing_context_pipeline.extraction.scene_context_extraction.graph_adapter.payload import (
...
from viewing_context_pipeline.extraction.scene_context_extraction.ondevice.extractor import image_content_for_model, load_images, move_inputs_to_model
...
SCHEMA_VERSION = "description-video-context/v1"
...
def qwen_infer(model: Any, processor: Any, images: list[Any], prompt: str, max_new_tokens: int, references: list[dict[str, Any]] | None = None) -> str:
    if references:
        validate_image_reference_alignment(len(images), references)
        content: list[dict[str, Any]] = []
        for image, reference in zip(images, references):
            content.extend(image_content_for_model([image]))
            content.append({"type": "text", "text": shot_reference_text(reference)})
    else:
        content = image_content_for_model(images) if images else []
    ...
    *,
    ...
    for fallback_idx, scene in enumerate(scenes):
        ...
        if multimodal:
            ...
            description = infer(load_images(image_paths), SCENE_PROMPT, scene_max_new_tokens, references).strip()
        else:
...
def write_description_outputs(context_path: str | Path, scene_path: str | Path, document: dict[str, Any], records: list[dict[str, Any]]) -> None:
    write_json_atomic(context_path, document)
