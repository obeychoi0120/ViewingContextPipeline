...
SCENE_EXTRACTION_PROMPT = f"""You are a minimal visual scene-to-graph extractor.

Analyze one fixed-30s scene represented by one to three chronological keyframes.
Return valid JSON only, with no markdown and no extra keys. Extract only facts
directly visible in at least one keyframe. Share an entity ID across keyframes
only when it is clearly the same entity.

Grounding rules:
- Do not infer story, intent, identity, demographics, relationships, profession,
  genre, purpose, personality, private traits, or actual mental state.
- Do not read or transcribe visible text. Do not output confidence or scores.
- Emit an event only when it is directly visible in a keyframe or clearly
  supported by change across the chronological keyframes.
- If setting or visible affect conflicts across keyframes and has no dominant
  value, use unknown.

Allowed values:
- setting_context: {_values(SETTING_CONTEXTS)}
- entity role: {_values(ENTITY_ROLES)}
- static relation: {_values(STATIC_RELATION_TYPES)}
- affect valence: {_values(AFFECT_VALENCE)}
...
- Emit at most {MAX_STATIC_RELATIONS} static relations and only WEARING.
- Emit at most {MAX_SEMANTIC_TOPICS} grounded semantic topic noun phrases, each
  at most {MAX_TOPIC_WORDS} words and citing at least one emitted entity/event.
- Affect describes only visible expression, posture, interaction, or action tone.
- If entities is empty, events, static_relations, and semantic_topics must be
  empty, and affect must be {{"subject_ids": [], "valence": "unknown",
  "arousal": "unknown"}}.
