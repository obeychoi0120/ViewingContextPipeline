...
def patch_paddlex_predictor() -> None:
    """Apply the local PaddleOCR character-dictionary compatibility patch."""
    ...
    try:
        ...
        raise RuntimeError("PaddleX is required for multimodal OCR") from exc

    def build_postprocess(self, **kwargs):
        if kwargs.get("name") != "CTCLabelDecode":
            raise ValueError(f"Unsupported OCR postprocess: {kwargs.get('name')}")
        path = kwargs.get("character_dict_path")
        if isinstance(path, list):
            path = path[0]
        with open(path, "r", encoding="utf-8") as handle:
            characters = [line.rstrip("\r\n") for line in handle]
        return CTCLabelDecode(character_list=characters)

    TextRecPredictor.build_postprocess = build_postprocess
...
def _looks_like_top_ui_text(text):
    compact = re.sub(r'\s+', '', str(text)).lower()
    top_ui_terms = ("검색", "search", "제목없음", "저장하지")
...
def process_ocr(frame_folder, ocr_json_path, *, model_root, score_thr=0.7, use_server_detector=True, min_height=0,
    ...
    try:
        ...
        raise RuntimeError(
            "OCR extraction requires the 'multimodal' optional dependencies"
        ) from exc

    patch_paddlex_predictor()
    base_model_dir = Path(model_root)
    rec_model_dir = base_model_dir / "official_models" / "korean_PP-OCRv5_mobile_rec"
    if use_server_detector:
        logger.info("Initializing PaddleOCR PP-OCRv5_server_det...")
        DET_MODEL_NAME = "PP-OCRv5_server_det"
        det_model_dir = base_model_dir / "official_models" / DET_MODEL_NAME

    else:
        logger.info("Initializing PaddleOCR PP-OCRv5_mobile_det...")
    ...
    if not det_model_dir.is_dir() or not rec_model_dir.is_dir():
        raise FileNotFoundError(
            f"OCR model directories are missing: detector={det_model_dir}, "
            f"recognizer={rec_model_dir}"
        )
