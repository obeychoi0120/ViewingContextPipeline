...
GPU와 Gemini 장비에서는 공유 `artifacts/preparation/cohort/`, `artifacts/preparation/resized_keyframes/`, `artifacts/preparation/source_assets/`를 배치하고 Gemini 결과를 `extraction/description/gemini`와 `extraction/graph/gemini`로 돌려보냅니다. 새 장면 형식에는 `.metadata`가 필요 없으며 경로·설정 변경으로 성공 결과를 재생성하지 않습니다. 이전 두 필드 파일을 전달할 때는 변환 전까지 원래 `.metadata`도 함께 보존해야 합니다. 생성 journal·cursor는 사용하지 않습니다.
