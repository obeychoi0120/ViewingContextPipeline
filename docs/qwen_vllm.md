...
진행률은 이번 실행의 요청을 기준으로 하며 `failed`는 각 항목의 최신 결과를 나타내며 재생성에 성공하면 `success`로 옮깁니다. `raw`는 파싱·검증 실패로 원문을 보존한 응답 수입니다. 토큰 종료·API 실패는 이 수에 포함하지 않습니다. 장면 `scene/s`에는 실패 결과도 포함합니다. Qwen과 Gemini 모두 장면별로 결과를 저장합니다. Graph 장면 JSONL은 `content_id`, `warning`, `scene_idx`, `scene_graph` 순서로 네 필드를 저장합니다. Description은 기존대로 `content_id`, `scene_idx`, `description`, `provenance`를 저장합니다. `.metadata`는 생성하지 않습니다. 이전 두 필드 파일의 장면 번호는 기존 `.metadata`에서 읽어 옮기고 새 파일 저장 후 해당 메타데이터를 삭제합니다. 이전 메타데이터가 없으면 장면 번호를 추측하지 않고 오류를 보고합니다.
