...
Qwen 장면 추출은 전체 대기 Scene의 현재 penalty 처리를 끝낸 다음, 검증·Repair에 실패한 Scene만 다음 penalty로 처리합니다. 각 penalty에서 Scene을 연속 공급하며 384개 단위로 완료를 기다리지 않습니다. 중단 후에는 저장된 시도를 건너뛰고 낮은 시도 차수부터 전체 Scene을 처리합니다. Summary는 기존 묶음별 재시도 순서를 유지합니다.
