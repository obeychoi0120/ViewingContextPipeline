...
```bash
CUDA_VISIBLE_DEVICES=0 python -m extraction extract-graph-scenes --model qwen --run-id "$RUN_ID" --schema prompts/graph_scene_v3.md --arm graph_qwen
CUDA_VISIBLE_DEVICES=0,2 python -m extraction summarize-graph --run-id "$RUN_ID" --schema prompts/graph_summary_v4.md --model qwen --arm graph_qwen
```
