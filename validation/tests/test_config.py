...
from vc_validation.config import load_config
from vc_validation.cli import preflight


def test_pilot_config_locks_user_count_and_protocol() -> None:
    path = Path(__file__).parents[1] / "config" / "pilot_1k.yaml"
    ...
    assert preflight(path)["config_valid"] is True


def test_canonical_config_locks_100k_and_bge_contract() -> None:
    config = load_config(Path(__file__).parents[1] / "config" / "canonical_100k.yaml")
