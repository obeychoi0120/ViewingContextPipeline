...
from vc_validation.cohort import largest_remainder_quotas, prepare_cohort, select_users, split_record
...
def test_prepare_cohort_needs_pairs_and_mp4_not_titles(tmp_path) -> None:
    ...
    catalog = [json.loads(line) for line in (config.output_dir / "cohort/catalog.jsonl").read_text(encoding="utf-8").splitlines()]
    assert all("title" not in row for row in catalog)
    assert (config.output_dir / "cohort/vce_smoke_selection.jsonl").is_file()
