...
def prepare_cohort(config: ExperimentConfig, *, probe: Callable[[Path], float] = probe_duration) -> dict[str, Any]:
    output = config.output_dir / "cohort"
    ...
    selection = [{"item_id": row["item_id"], "content_id": row["content_id"], "source_video_path": row["source_video_path"], "url": f"microlens://100k/{row['item_id']}"} for row in catalog]
    smoke_users = sorted(sequences, key=lambda row: _stable_key(config.cohort.seed, row["user_id"]))[:config.cohort.smoke_user_count]
    smoke_ids = {item for row in smoke_users for item in row["sequence"]}
    ...
    atomic_write_jsonl(output / "vce_selection.jsonl", selection)
    atomic_write_jsonl(output / "vce_smoke_selection.jsonl", [row for row in selection if row["item_id"] in smoke_ids])
