...
from src.common.gemini import make_extraction_config, parse_json_response
...
SCENE_CONTEXT_INPUT_SUFFIX = "_ref.jsonl"
...
def load_scene_context_rows(
    ref_jsonl_dir: str | Path,
    content_ids: Iterable[str] | None = None,
    *,
    multimodal: bool = False,
) -> tuple[list[dict[str, Any]], int]:
    input_dir = Path(ref_jsonl_dir)
    if not input_dir.is_dir():
        raise ValueError(f"local Ref JSONL directory not found: {input_dir}")
    ...
    for ref_path in ref_paths:
        content_id = ref_path.name[: -len(SCENE_CONTEXT_INPUT_SUFFIX)]
        source_uri = str(ref_path)
        records = parse_ref_jsonl(ref_path.read_text(encoding="utf-8"), source_uri)
        seen_scene_idxs: set[int] = set()
        content_rows: list[dict[str, Any]] = []
        for line_number, record in records:
            if record.get("_type") == "video_metadata":
                continue
            scene = ref_scene_to_context(
                record,
                source_uri,
                line_number,
        ...
        if not content_rows:
            raise ValueError(f"{source_uri}: Ref JSONL contains no scene records")
        rows.extend(sorted(content_rows, key=lambda row: row["scene_idx"]))
    return rows, len(ref_paths)


def parse_ref_jsonl(text: str, source_uri: str) -> list[tuple[int, dict[str, Any]]]:
...
def reference_failure_path(
    scene_context_path: str | Path,
    content_id: str,
) -> Path:
    scene_path = Path(scene_context_path)
    if scene_path.parent.name == "scene_context_graph_ref":
...
def extract_scene_contexts_ref(
    scene_context_rows: list[dict[str, Any]],
    output_path: str | Path,
    client: Any,
    ...
    with tqdm(total=len(grouped_rows), desc="Contents", unit="content", disable=not show_progress) as progress:
        for content_id, rows in grouped_rows.items():
            ...
            current_failure_path = reference_failure_path(
                current_output_path,
                content_id,
            )
            ...
            successful_records, failure_records = load_reference_state(
                content_id=content_id,
                rows=rows,
            ...
            if reference_state_complete(
                rows,
                successful_records,
                failure_records,
            ):
                successful_records, failure_records = persist_reference_state(
                    content_id=content_id,
                    rows=rows,
                    output_path=current_output_path,
                    failure_path=current_failure_path,
                ...
                if write_reference_context(
                    ...
                    tqdm.write(
                        f"[skip] {content_id}: existing Reference state is terminal"
                    )
...
def extract_scene_context_row(
    *,
    ...
    try:
        target_graph, warnings = extract_scene_context_ref(
            client=client,
...
def output_path_for_content(output_path: str | Path, output_dir: str | Path | None, content_id: str) -> Path:
    if output_dir:
        return Path(output_dir) / f"{content_id}_scene_context_ref.jsonl"
    return Path(output_path)


def scene_context_ref_output_dir(
    output_dir: str | Path,
    shot_interval: str = "fixed_15s",
    multimodal: bool = False,
) -> Path:
...
def video_context_graph_ref_output_dir(
    output_dir: str | Path,
    shot_interval: str = "fixed_15s",
    multimodal: bool = False,
) -> Path:
...
def context_path_for_content(
    ...
    if output_dir is not None:
        ...
        output_root = (
            scene_dir.parent
            if scene_dir.name == "scene_context_graph_ref"
            else scene_dir
        )
        profile_dir = output_root / "video_context_graph_ref"
    else:
        raise ValueError("video_context_output_dir or scene_context_path is required")
    return profile_dir / f"{content_id}_context_graph_ref.json"
