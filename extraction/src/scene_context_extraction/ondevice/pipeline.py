...
SUPPORTED_MODEL_FAMILIES = frozenset({"qwen3_vl", "ministral3"})
MODEL_FAMILY_POSTFIXES = {
    "qwen3_vl": "qwen",
    "ministral3": "mistral",
}
GRAPH_RECORD_FIELDS = {
...
def video_id_from_content_id(content_id: str) -> str:
...
def scene_context_job_video_id(job: SceneContextJob) -> str:
    return job.video_id or video_id_from_url(job.url) or video_id_from_content_id(job.content_id)


def required_scene_context_input_errors(job: SceneContextJob) -> list[str]:
    errors: list[str] = []
    if not has_scene_context_source(job):
        errors.append(f"missing or empty ref_jsonl: {job.ref_jsonl}")
...
def has_scene_context_source(job: SceneContextJob) -> bool:
    ref_path = Path(job.ref_jsonl) if job.ref_jsonl else None
    if ref_path and ref_path.exists() and ref_path.stat().st_size > 0:
        try:
            _, scenes = split_metadata_and_scenes(read_jsonl(ref_path))
        except (OSError, json.JSONDecodeError):
            scenes = []
        if scenes:
            return True
...
def manifest_row_to_job(
    ...
    manifest_ref_jsonl = str(row.get("ref_jsonl") or "").strip()
    timestamp_json = str(row.get("filtered_timestamp_json") or row.get("timestamp_json") or "").strip()
    content_id = str(row.get("content_id") or row.get("name") or "").strip()
    url = str(row.get("url") or "").strip()
    video_id = str(row.get("video_id") or "").strip() or video_id_from_url(url)
    if not content_id and manifest_ref_jsonl:
        content_id = Path(manifest_ref_jsonl).stem.replace("_ref", "")
    if not content_id:
        raise ValueError("manifest row is missing content_id")

    save_path = infer_save_path(row, manifest_ref_jsonl, timestamp_json, content_id)
...
def default_scene_context_path(
    ...
    postfix = model_family_postfix(model_family)
    mode = normalize_shot_interval(shot_interval)
    return (
        output_save_path()
        / viewing_context_relative_path(multimodal, mode)
        / f"scene_context_graph_{postfix}"
        / f"{content_id}_scene_context.jsonl"
    )


def model_family_postfix(model_family: str) -> str:
    try:
        return MODEL_FAMILY_POSTFIXES[model_family]
    except KeyError as exc:
        supported = ", ".join(sorted(SUPPORTED_MODEL_FAMILIES))
        raise ValueError(
            f"unsupported MODEL_FAMILY {model_family!r}; expected one of: {supported}"
        ) from exc


def default_ref_jsonl_path(
    content_id: str,
    shot_interval: str = "fixed_15s",
) -> Path:
    return (
...
def default_metadata_path(content_id: str) -> Path:
    return output_save_path() / "asset" / "metadata" / f"{content_id}.json"
...
def infer_save_path(row: dict[str, object], ref_jsonl: str, timestamp_json: str, content_id: str) -> str:
    save_path = str(row.get("save_path") or "").strip()
    if save_path:
        return save_path
    if ref_jsonl:
        return str(Path(ref_jsonl).parent)
    if timestamp_json:
        return str(Path(timestamp_json).parent.parent)
    return str(assets_save_path() / content_id)


def infer_assets_path(row: dict[str, object], ref_jsonl: str, timestamp_json: str, save_path: str) -> Path:
    assets_path = str(row.get("assets_path") or "").strip()
    if assets_path:
        return Path(assets_path)
    if ref_jsonl:
        return Path(ref_jsonl).parent / "assets"
    if timestamp_json:
        return Path(timestamp_json).parent
    return Path(save_path) / "assets"
...
def read_manifest(
    ...
    return [
        manifest_row_to_job(row, model_family, shot_interval, multimodal)
...
def load_scene_context_source(job: SceneContextJob) -> list[dict[str, Any]]:
    ...
    if ref_path and ref_path.exists():
        ...
        if scenes:
            return scenes
        raise ValueError(f"{job.content_id}: ref_jsonl has no scene records")
    raise FileNotFoundError(f"{job.content_id}: graph context requires ref_jsonl")
...
def ondevice_output_root(scene_context_path: Path) -> Path:
    ...
    if scene_context_dir == "scene_context_graph" or scene_context_dir.startswith("scene_context_graph_"):
        ...
        if mode_root.name in {"fixed_15s", "fixed_30s", "shot_wise"}:
...
def ondevice_model_postfix(scene_context_path: Path) -> str | None:
    ...
    if postfix not in MODEL_FAMILY_POSTFIXES.values():
    ...
    try:
        with contextlib.redirect_stdout(init_log), contextlib.redirect_stderr(init_log):
            ...
            model, processor = init_vlm_model(
                model_path,
                model_family_from_config(vlm_config),
            )
...
def init_ministral3_model(model_path: str):
    import torch
...
def model_family_from_config(vlm_config: dict[str, Any]) -> str:
    model_family = str(vlm_config.get("MODEL_FAMILY", "qwen3_vl")).strip()
    if model_family not in SUPPORTED_MODEL_FAMILIES:
        raise ValueError(
            f"unsupported MODEL_FAMILY {model_family!r}; "
            f"expected one of {sorted(SUPPORTED_MODEL_FAMILIES)}"
        )
    return model_family


def init_vlm_model(model_path: str, model_family: str):
    if model_family == "qwen3_vl":
        return init_qwen3vl_model(model_path, use_fc_patch=True)
    if model_family == "ministral3":
        return init_ministral3_model(model_path)
    raise ValueError(
        f"unsupported MODEL_FAMILY {model_family!r}; "
        f"expected one of {sorted(SUPPORTED_MODEL_FAMILIES)}"
    )
