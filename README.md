...
기본 네 Arm 실험은 title 표현을 시각 기반 표현으로 교체했을 때의 효과를 측정합니다. 추가한 **V1 Metadata + Ours Graph (LoRA)** 행의 실행 방법과 입력 조건은 [V1 재현 가이드](docs/metadata_ours_v1.md)에 있습니다. 온라인 추천 효과와 모델의 일반적인 우열은 검증 범위에 포함하지 않습니다.
